...
static const uint8_t write_asset_data[ETSS_MAX_ASSET_SIZE] = {0xBF};
static uint8_t read_asset_data[ETSS_MAX_ASSET_SIZE] = {0};
...
static void tfm_etss_test_2008(struct test_result_t *ret);
static void tfm_etss_test_2009(struct test_result_t *ret);
static void tfm_etss_test_2010(struct test_result_t *ret);
...
static void tfm_etss_test_2018(struct test_result_t *ret);
static void tfm_etss_test_2019(struct test_result_t *ret);
static void tfm_etss_test_2020(struct test_result_t *ret);
//static void tfm_etss_test_2021(struct test_result_t *ret);
static void tfm_etss_test_2022(struct test_result_t *ret);

static struct test_t psa_etss_s_tests[] = {
    {&tfm_etss_test_2001, "TFM_ETSS_TEST_2001",
     "Set interface"},
    {&tfm_etss_test_2002, "TFM_ETSS_TEST_2002",
    ...
    {&tfm_etss_test_2007, "TFM_ETSS_TEST_2007",
     "Get interface with zero data length"},
    {&tfm_etss_test_2008, "TFM_ETSS_TEST_2008",
     "Get interface with invalid UIDs"},
    {&tfm_etss_test_2009, "TFM_ETSS_TEST_2009",
     "Get interface with invalid data lengths and offsets"},
    {&tfm_etss_test_2010, "TFM_ETSS_TEST_2010",
     "Get interface with NULL data pointer"},
    {&tfm_etss_test_2011, "TFM_ETSS_TEST_2011",
     "Get info interface with write once UID"},
    {&tfm_etss_test_2012, "TFM_ETSS_TEST_2012",
    ...
    {&tfm_etss_test_2017, "TFM_ETSS_TEST_2017",
     "Remove interface with invalid UID"},
    {&tfm_etss_test_2018, "TFM_ETSS_TEST_2018",
     "Block compaction after remove"},
    {&tfm_etss_test_2019, "TFM_ETSS_TEST_2019",
     "Multiple partial gets"},
    {&tfm_etss_test_2020, "TFM_ETSS_TEST_2020",
     "Multiple sets to same UID from same thread"},
    {&tfm_etss_test_2022, "TFM_ETSS_TEST_2022",
...
void register_testsuite_s_psa_etss_interface(struct test_suite_t *p_test_suite)
{
    ...
    list_size = (sizeof(psa_etss_s_tests) / sizeof(psa_etss_s_tests[0]));

    set_testsuite("external trusted secure storage S interface tests (TFM_ETSS_TEST_2XXX)",
                  psa_etss_s_tests, list_size, p_test_suite);
}
...
static void tfm_etss_test_2001(struct test_result_t *ret)
{
    ...
    err = tfm_etss_set(INVALID_UID, data_len, write_data, flags);
...
static void tfm_etss_test_2002(struct test_result_t *ret)
{
    ...
    /* Set with no flags */
    err = tfm_etss_set(WRITE_ONCE_UID, data_len, write_data, flags);
    if (err == ETSS_SUCCESS) {
    ...
    err = tfm_etss_set(uid, data_len, write_data, INVALID_FLAG);
    if (err != ETSS_ERR_NOT_SUPPORTED) {
...
static void tfm_etss_test_2003(struct test_result_t *ret)
{
    ...
    err = tfm_etss_set(uid, data_len, NULL, flags);
    if (err != ETSS_SUCCESS) {
...
static void tfm_etss_test_2005(struct test_result_t *ret)
{
    ...
    err = tfm_etss_set(uid, write_len, write_data, flags);
    if (err != ETSS_ERR_NOT_PERMITTED) {
...
static void tfm_etss_test_2006(struct test_result_t *ret)
{
    ...
    /* Get the entire data */
    err = tfm_etss_get(uid, offset, data_len, read_data + HALF_PADDING_SIZE,
                        &read_data_len);
    if (err != ETSS_SUCCESS) {
        TEST_FAIL("Get should not fail");
        return;
...
static void tfm_etss_test_2008(struct test_result_t *ret)
{
    ...
    err = tfm_etss_get(uid, offset, data_len, read_data + HALF_PADDING_SIZE,
                        &read_data_len);
    if (err != ETSS_ERR_DOES_NOT_EXIST) {
    ...
    err = tfm_etss_get(INVALID_UID, offset, data_len,
...
static void tfm_etss_test_2010(struct test_result_t *ret)
{
    ...
    err = tfm_etss_get(uid, offset, 0, NULL, &read_data_len);
    if (err != ETSS_SUCCESS) {
...
static void tfm_etss_test_2011(struct test_result_t *ret)
{
    ...
    err = tfm_etss_get_info(uid, &info);
    if (err != ETSS_SUCCESS) {
...
static void tfm_etss_test_2013(struct test_result_t *ret)
{
    ...
    err = tfm_etss_get_info(INVALID_UID, &info);
    if (err != ETSS_ERR_INVALID_ARGUMENT) {
...
static void tfm_etss_test_2015(struct test_result_t *ret)
{
    ...
    err = tfm_etss_get(uid, offset, data_len, read_data, &read_data_len);
    if (err != ETSS_ERR_DOES_NOT_EXIST) {
...
static void tfm_etss_test_2018(struct test_result_t *ret)
{
    ...
    /* Set UID 1 */
    err = tfm_etss_set(uid_1, sizeof(write_data_1), write_data_1, flags);
    if (err != ETSS_SUCCESS) {
    ...
    /* Set UID 2 */
    err = tfm_etss_set(uid_2, data_len_2, write_data_2, flags);
    if (err != ETSS_SUCCESS) {
     ...
     */
    err = tfm_etss_get(uid_2, offset, data_len_2,
                        read_data + HALF_PADDING_SIZE, &read_data_len);
    if (err != ETSS_SUCCESS) {
...
static void tfm_etss_test_2020(struct test_result_t *ret)
{
    ...
    err = tfm_etss_set(uid, sizeof(write_data_1), write_data_1, flags);
    if (err != ETSS_SUCCESS) {
    ...
    err = tfm_etss_set(uid, sizeof(write_data_2), write_data_2, flags);
    if (err != ETSS_SUCCESS) {
    ...
    err = tfm_etss_set(uid, sizeof(write_data_3), write_data_3, flags);
    if (err != ETSS_SUCCESS) {
        TEST_FAIL("Third set should not fail");
        return;
    }

    err = tfm_etss_get(uid, offset, sizeof(write_data_3), read_data,
                        &read_data_len);
    if (err != ETSS_SUCCESS) {
        TEST_FAIL("Get should not fail");
        return;
...
//
//    support_flags = psa_etss_get_support();
//    if (support_flags != 0) {
...
static void tfm_etss_test_2022(struct test_result_t *ret)
{
    ...
    const uint32_t test_asset_sizes[TEST_1022_CYCLES] = {
        ETSS_MAX_ASSET_SIZE >> 2,
        ETSS_MAX_ASSET_SIZE >> 1,
        ETSS_MAX_ASSET_SIZE};
    ...
    for (cycle = 0; cycle < TEST_1022_CYCLES; cycle++) {
        ...
        err = tfm_etss_get(uid, 0, data_size, read_asset_data,
